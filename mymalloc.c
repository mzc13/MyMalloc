			...
			if (parity == 0) {
				if (nextIndex == 0) {
					...
					if (size < (4093 - currentIndex)) {
						// If there is space for another allocation, leave space for it
						*meta = 0x1000 | (currentIndex + (unsigned short)size + 2);
						*nextMeta = 0;
						return myBlock + currentIndex + 2;
					}
					else if (size < (4095 - currentIndex)) {
						// If there is not space for another allocation, let this item take up all the space
						*meta = 0x1000;
						return myBlock + currentIndex + 2;
					}
					else {
					...
					if (size < (nextIndex - (currentIndex + 4))) {
						...
						*nextMeta = nextIndex;
						...
						// If there is not space for another allocation, let this item take up the entire block
